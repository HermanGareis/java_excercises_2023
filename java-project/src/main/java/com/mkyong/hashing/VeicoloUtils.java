...
/**
 *
 * A utility class containing methods for working with the {@link Veicolo}
 * class. Implements the {@link StreamUtiles} interface for functional
 * programming operations on collections.
 *
...
public class VeicoloUtils implements StreamUtiles {
	...
	/**
	 * Applies the specified {@link Consumer} to each {@link Veicolo} object in the
	 * given List.
	 *
	 * @param list     the list of {@link Veicolo} objects to apply the consumer to
	 * @param consumer the consumer to apply to each {@link Veicolo} object in the
	 *                 list
	 **/
	...
	/**
	 * Filters the given List of {@link Veicolo} objects using the specified
	 * {@link Predicate}, and prints out the brand and power of each resulting
	 * {@link Veicolo} object. Also imlements parallelStream for multithreading.
	 * 
	...
	public void applyPredicate(List<Veicolo> list, Predicate<Veicolo> predicate) {
		...
		// filters the stream based on the given Predicate and then prints out the
		// results for each object using a lambda expression.
		list.parallelStream().filter(predicate).forEach(veicolo -> System.out
				.println(veicolo.getMarca() + " is powerful: " + veicolo.getPotenza() + " (predicate)"));

	}

	/**
	 * Maps the specified {@link Function} to each {@link Veicolo} object in the
	 * given List, and prints out the resulting {@code String} objects.
	 * 
	 * @param list     the list of {@link Veicolo} objects to map
	 * @param function the function to apply to each {@link Veicolo} object in the
	 *                 list
	 **/
	public void applyFunction(List<Veicolo> list, Function<Veicolo, String> function) {
		...
		list.parallelStream().map(function.andThen(result -> result + " (function)")).forEach(System.out::println);
	}
